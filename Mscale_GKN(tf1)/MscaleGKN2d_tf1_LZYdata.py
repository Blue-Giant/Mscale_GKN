...
def solve_multiScale_operator(R):
    ...
    if R['model2input'] == 'Fourier_DNN':
        W2Input, B2Input = DNN_base.Xavier_init_NN_Fourier(2*input_dim + 1, out_dim, hidden2input, flag2Input)
    else:
        W2Input, B2Input = DNN_base.Xavier_init_NN(2*input_dim + 1, out_dim, hidden2input, flag2Input)
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.compat.v1.variable_scope('vscope2train', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            AepsX2train = tf.compat.v1.placeholder(tf.float32, name='AepsX2train', shape=[batch2train, point_samples2train, 1])
            ...
            for i_batch in range(batch2train):
                ...
                sample_train_A = tf.reshape(Aeps2train[i_batch, :, :], [-1, 1])
    ...
    train_data_AepsX = train_dataSet['meshAx']
    train_data_AepsY = train_dataSet['meshAy']
    ...
    trainData2AepsX = np.reshape(train_data_AepsX, newshape=[shape2data[0], res2train * res2train])
    trainData2AepsY = np.reshape(train_data_AepsY, newshape=[shape2data[0], res2train * res2train])
    ...
    normalize_trainData2Aeps = trainData2Aeps_normalizer.encode(trainData2Aeps, bool_var=False)

    trainData2AepsX_normalizer = DNN_Class_base.np_GaussianNormalizer(trainData2AepsX)
    normalize_trainData2AepsX = trainData2AepsX_normalizer.encode(trainData2AepsX, bool_var=False)

    trainData2AepsY_normalizer = DNN_Class_base.np_GaussianNormalizer(trainData2AepsY)
    normalize_trainData2AepsY = trainData2AepsY_normalizer.encode(trainData2AepsY, bool_var=False)
    ...
    shuffle_norm_testData2AepsY = normalize_testData2AepsY[:, disorder_index2test_data]
    ...
    with tf.Session(config=config) as sess:
        ...
        for i_epoch in range(R['max_epoch'] + 1):
            ...
            if i_epoch % 100 == 0:
                ...
                sampleBatch2Train_AepsX = normalize_trainData2AepsX[index2sample_batch, :]
                sampleBatch2Train_AepsY = normalize_trainData2AepsY[index2sample_batch, :]
            ...
            if i_epoch % 100 == 0:
                ...
                unn2test, test_mse_tmp, test_rel_tmp, test_l2rel_tmp = sess.run(
                    [UMGKN2test, test_mse, test_rel, test_l2rel], feed_dict={
                        XY2test: mesh2test, Aeps2test: Test_Aeps, AepsX2test: Test_AepsX, AepsY2test: Test_AepsY,
                        U2test: Test_U})
...
if __name__ == "__main__":
    ...
    # R['mesh_number2train'] = 41
    # R['mesh_number2test'] = 41

    # R['mesh_number2train'] = 49
    # R['mesh_number2test'] = 49

    R['mesh_number2train'] = 61
    R['mesh_number2test'] = 61

    # R['mesh_number2train'] = 81
    # R['mesh_number2test'] = 81

    # R['mesh_number2train'] = 121
    # R['mesh_number2test'] = 121
